...
message_quest_2 = "Ты зашёл так далеко! Но здесь ты не пройдешь. " \
                  ...
                  "\n\n(для ТЕСТИРОВЩИКОВ вот QR-код с этой же ссылкой, на случай если вк будет блокировать ссылку)"
...
while True:
    try:
        ...
        def create_keyboard(UserResponse):
            ...
            elif UserResponse == 'key_start':
                UserKeyboard = VkKeyboard(one_time=True)
                UserKeyboard.add_button('Начать', color=VkKeyboardColor.PRIMARY)
        ...
        def vic_indicator(user_id):
            ...
            if dictionary_vic_question[user_id] == 1:
                text_question1 = victorina_mass[0][1]

            elif dictionary_vic_question[user_id] == 2:
                text_question1 = victorina_mass[0][3]

            send_message(vk_session, event.user_id,
                         message=text_question1,
                ...
                if event.type == VkEventType.MESSAGE_NEW:
                    ...
                    try:
                        ...
                        if event.from_user and not event.from_me:
                            if ((response == "1") or (response == "2") or (response == "3") or (response == "4")) and \
                                ...
                                if result is not None:  # Выполняется если польщователь прошел викторину
                                    message_result = generationMessage.messageResult(
                                        result) + "\nТеперь тебе доступен КВЕСТ для которого тебе пригодится этот QR-код."
                                    keyboard = create_keyboard("расписание")
                                ...
                                if response == "sql":
                                    ...
                                    if dictionary_quest[event.user_id] == 1:
                                        ...
                                        if (response.find("10 15") > -1) or (response.find("10-15") > -1):
                                            ...
                                            send_message(vk_session, event.user_id, message=message_quest_2, UserAttachment="photo-192914903_457239053")
                            ...
                            elif event.user_id and (event.user_id in dictionary_contest_vote):
                                if response == "Приступить":
                                    send_message(vk_session, event.user_id,
                                                 message='тест')
                                    ...
                                    and not (event.user_id in dictionary_quest_indicator) and not (event.user_id in dictionary_contest_vote):
                                ...
                                if response == "начать":
                                    ...
                                    if dictionary_quest[event.user_id] == 1:
                                        ...
                                        send_message(vk_session, event.user_id, message=message_quest_2,
                                                     UserAttachment="photo-192914903_457239053",
                                                     UserKeyBoard=keyboard)
